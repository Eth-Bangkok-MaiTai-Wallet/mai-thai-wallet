...
import { z } from 'zod';
import { generateQueueWithdrawalTxBundle } from '@/lib/tools/restakeTxBuilder';
...
export async function POST(req: Request) {
    ...
    console.log("----Queue Unestake Agent----:", messages);
    ...
    const { toolCalls } = await generateText({
        ...
        tools: {
            ...
            queue_unrestake: tool({
              description: 'A tool for creating a transaction object to queue a eigenlayer withdrawal. Use this tool when classification result is queue_unrestake. Parameter is user wallet address',
              parameters: z.object({userWallet: z.string()}),
              execute: async ({userWallet}) => generateQueueWithdrawalTxBundle(userWallet)
            }),
            answer: tool({
